...
export default function Login() {
    ...
    const verifyOtp = async () => {
        ...
        try {
            ...
            if (!['OPERATOR', 'ADMIN'].includes(role)) {
                alert('This account is not authorized for Operator access.');
                setLoading(false);
                return;
            }
    ...
    return (
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#F5F2EC', padding: 24 }}>
            <div style={{ background: '#0A0A0F', color: 'white', borderRadius: 12, padding: '8px 20px', marginBottom: 28, fontWeight: 800, fontSize: 20 }}>
                🅿 ParkIQ Operator
            </div>
            ...
            <div style={{ background: 'white', borderRadius: 16, padding: 28, width: '100%', maxWidth: 380, boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
                <h2 style={{ fontWeight: 800, fontSize: 22, marginBottom: 6 }}>
                    {step === 'phone' ? 'Operator Login' : 'Enter OTP'}
                </h2>
                ...
                {devOtp && (
                    <div style={{ background: '#FFF3CD', borderRadius: 8, padding: '8px 12px', marginBottom: 16, fontSize: 13, fontWeight: 600 }}>
                        {devOtp}
                    </div>
                )}
                ...
                {step === 'phone' ? (
                    <>
                        ...
                        <div style={{ display: 'flex', border: '1.5px solid #ddd', borderRadius: 10, overflow: 'hidden', marginBottom: 20 }}>
                    ...
                    <>
                        ...
                        <input
                            ...
                            maxLength={6}
                            type="tel"
                            style={{ width: '100%', padding: 14, borderRadius: 10, border: '1.5px solid #ddd', fontSize: 22, textAlign: 'center', letterSpacing: 8, marginBottom: 20, boxSizing: 'border-box', outline: 'none' }}
                        />
                        <button
                            onClick={verifyOtp}
                            disabled={loading}
